    ...
    RequestContextViewModel requestContextViewModel)
{
    public async Task<FunctionResponse<List<BankListItemViewModel>>> ListAsync(BankListRequestViewModel request)
    {
        ...
        var response = banks
            .OrderBy(x => x.Name)
            .Select(bank => new BankListItemViewModel
            {
                Id = bank.Id.ToString(),
                ...
                Accounts = accounts[bank.Id]
                    .OrderBy(p=>p.Name)
                    .Select(x => new BankAccountListItemViewModel
                    {
                        Id = x.Id.ToString(),
                        Name = x.Name,
                        NickName = x.NickName,
    ...
    public async Task<FunctionResponse<List<BankDefinitionListItemViewModel>>> ListDefinitionsAsync()
    {
        ...
        var response = definitions.OrderBy(x => x.Name).Select(x => new BankDefinitionListItemViewModel
        {
            Id = x.Id.ToString(),
            Name = x.Name,
            LogoName = x.LogoName,
    ...
    public async Task<FunctionResponse<BankListItemViewModel>> CreateAsync(BankUpsertRequestViewModel request)
    {
        ...
        {
            UserId = userId,
            Name = string.IsNullOrEmpty(request.BankDefinitionId)?null:request.Name,
