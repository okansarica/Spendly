    ...
    RequestContextViewModel requestContextViewModel)
{
        ...
        ReportsOverviewRequestViewModel request)
    {
        ...
        var categoryChanges = allCategoryIds
            .Select(id => BuildCategoryChange(id, currentTotals, previousTotals, categoryLookup))
        ...
        AccountsOverviewRequestViewModel request)
    {
        ...
        var accounts = allAccountIds
            .Select(id => BuildAccountListItem(id, currentTotals, previousTotals, accountLookup))
            .OrderByDescending(x => x.CurrentMonthToDateTotal)
            .ToList();
    ...
    private async Task<AccountComparisonViewModel> BuildAccountComparison(
        DateTime startLocal,
        DateTime endLocal,
        DateTime nowLocal,
        ObjectId userId,
    ...
    private async Task<List<DailyCategoryAccountExpense>> GetDailySummariesAsync(
        ObjectId userId,
        DateTime startUtc,
        DateTime endUtc,
    ...
    private static ReportSummaryViewModel BuildSummary(decimal currentTotal, decimal previousTotal)
    {
        ...
        var trend = difference > 0 ? Constants.Reports.TrendIncrease : difference < 0 ? Constants.Reports.TrendDecrease : Constants.Reports.TrendNeutral;
    ...
    private static ReportCategoryChangeViewModel BuildCategoryChange(
        ObjectId categoryId,
    ...
    private static AccountListItemViewModel BuildAccountListItem(
        ObjectId accountId,
        Dictionary<ObjectId, decimal> currentTotals,
        Dictionary<ObjectId, decimal> previousTotals,
